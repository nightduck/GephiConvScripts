...
while len(elems.ways) > 0:  
    ...
    for ref in children[1:-1]:
        if ref != lastPoint.ref:    #Sometimes the way will list duplicate nodes. This is a problem
            ...
            edgeExists = lambda x, yref: len(filter(lambda e: edges[e].getPair(x.ref) == yref, x.edges)) > 0
...
rem = []
print "Concatenating segmented highways"
...
centralNode = int(raw_input("guess, enter 1: "))
#For california, 65362185 is Lombard and Leavenworth

print "Filtering out rouge street groups"
if centralNode != 0:
    ...
    if centralNode == 1:
        centralNode = elems.nodes.getKeys[0]
    q = [centralNode]
    while len(q) > 0:
        current = q.pop()
